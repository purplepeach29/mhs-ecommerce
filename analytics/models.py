...
class ObjectViewed(models.Model):
	...
	object_id = models.PositiveIntegerField() #userid, prod id, order id
	content_object = GenericForeignKey('content_type', 'object_id') #prod instance
	...
	class Meta:
		ordering =['-timestamp'] #most recent stored first
...
def post_save_session_receiver(sender, instance, created, *args, **kwargs):
	if created:
		qs = UserSession.objects.filter(user = instance.user).exclude(id = instance.id)#ended = False, active = False
		for i in qs:
			i.end_session()
